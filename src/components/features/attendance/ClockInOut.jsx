...
const ClockInOut = () => {
  ...
  useEffect(() => {
    const fetchTodayAttendance = async () => {
      try {
        ...
        } else if (sampleAttendance.arrival_time && sampleAttendance.departure_time) {
  ...
  const getUserDisplayName = () => {
    ...
    return firstName && surname ? `${firstName} ${surname}` : user?.email || 'User';
  };
      ...
      {attendanceStatus === 'not_started' && (
        <Alert severity="info" sx={{ mt: 3 }}>
          <Typography variant="body2">
            <strong>Instructions:</strong> Click "Clock In" when you arrive at work. 
            Remember to clock out when you finish your workday. Standard working hours are 8 hours per day.
          </Typography>
        </Alert>
      )}
    </Box>
